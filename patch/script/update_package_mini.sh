...
sed -i 's/$(VERSION_DIST_SANITIZED)/$(shell TZ=UTC-8 date +%Y%m%d)-OpenWrt-Mini/g' include/image.mk
sed -i 's/invalid/# invalid/g' package\network\services\samba36\files\smb.conf.template
